//! Оценка направления по бинауральным признакам.
//!
//! Заменяет прежнюю широкополосную кросс-корреляцию, у которой было две
//! принципиальные проблемы. Во-первых, сумма бралась только по перекрывающимся
//! отсчётам, и число слагаемых падало с ростом сдвига — пик систематически
//! тянуло к нулю, то есть к центру экрана. Во-вторых, один пик на весь чанк
//! давал одно усреднённое направление между одновременно звучащими
//! источниками, то есть направление, где нет никого.
//!
//! Здесь вместо этого:
//!
//! * отбеливание кросс-спектра (PHAT) — устойчиво к реверберации, и смещения
//!   к нулю в нём нет по построению;
//! * SRP-PHAT: отклик считается сразу на сетке азимутов, без промежуточного
//!   обратного преобразования и без привязки к целым сэмплам;
//! * деляция — источники снимаются по одному, и бины, объяснённые найденным
//!   направлением, исключаются из поиска следующего.
//!
//! Знаковое соглашение всюду: положительный ITD и положительный азимут —
//! источник справа.
//!
//! # Что здесь принципиально не решается
//!
//! Два одновременных широкополосных источника с одинаковым спектром по одному
//! кадру не разделяются никак: фаза в каждом бине становится их смесью и не
//! принадлежит ни одному из них. Деляция помогает ровно тогда, когда источники
//! занимают разные участки спектра — как шаг и выстрел. Поэтому наружу
//! отдаётся не только список пиков, но и весь отклик [`Localizer::response`]:
//! честно показанная неопределённость полезнее одной уверенной стрелки,
//! которая может указывать в пустоту.

use crate::dsp::calib::{Calibration, sample_lut};
use crate::dsp::fft::{Complex, Fft};

/// Максимум одновременно отслеживаемых источников.
pub const MAX_PEAKS: usize = 4;

/// Шаг сетки азимутов, градусы.
pub const HIST_STEP_DEG: f32 = 2.5;
/// Число узлов отклика, покрывающих [-90, +90].
pub const HIST_BINS: usize = 73;

/// Сглаживание отклика, в узлах сетки.
const HIST_SIGMA_BINS: f32 = 1.6;

/// Минимальный разнос между источниками, градусы.
///
/// Взят с запасом: точность направления по стерео всё равно хуже этого, а два
/// противника в пределах 20 градусов друг от друга для игрока находятся в
/// одной стороне. Слишком мелкий порог вместо этого дробил один источник на
/// несколько меток, когда подсказки слегка расходились.
const MIN_PEAK_SEPARATION_DEG: f32 = 20.0;

/// Пик должен набрать хотя бы такую долю от главного максимума.
const PEAK_REL_THRESHOLD: f32 = 0.28;

/// Порог энергии бина относительно пика кадра. Ниже — утечка окна и шумовая
/// полка, где фаза и разница уровней случайны: такие бины голосуют за
/// «примерно центр» и размывают картину.
const BIN_FLOOR_REL: f32 = 0.02;

/// Доверие к ILD относительно ITD. ILD сильнее зависит от конкретной HRTF,
/// поэтому при равной энергии в полосах приоритет у разницы времени.
const ILD_TRUST: f32 = 0.7;

/// Разброс измерения ILD, дБ: и модель, и калибровка приблизительны.
const ILD_SIGMA_DB: f32 = 3.0;

/// Минимальная когерентность, при которой найденный пик считается источником,
/// а не случайным совпадением фаз.
const MIN_COHERENCE: f32 = 0.22;

/// Насколько хорошо азимут должен объяснять фазу бина, чтобы забрать его себе.
const ASSIGN_THRESHOLD: f32 = 0.5;

/// Меньше этого числа свободных бинов искать следующий источник бессмысленно.
...
pub struct Peak {
    ...
    pub azimuth_deg: f32,
    /// Доля энергии, собранная этим источником, 0..1 (у сильнейшего 1.0).
    pub weight: f32,
    /// Насколько надёжно направление, 0..1.
    pub confidence: f32,
}
...
pub struct Localizer {
    ...
    /// Кривые ILD, пересчитанные с калибровочной сетки на сетку отклика:
    /// `[полоса * HIST_BINS + узел]`.
    ild_curves: Vec<f32>,

    /// Таблицы наведения: `cos`/`sin` фазы `2*pi*k*tau(theta)/n` для каждой
    /// пары (узел сетки азимутов, частотный бин полосы ITD). Считаются один раз.
    steer_cos: Vec<f32>,
    steer_sin: Vec<f32>,

    cross: Vec<Complex>,
    /// Отбелённый кросс-спектр в полосе ITD.
    psi: Vec<Complex>,
    /// Энергетический вес бина полосы ITD.
    bin_w: Vec<f32>,
    /// Бин ещё не отдан ни одному источнику.
    bin_live: Vec<bool>,
...
impl Localizer {
    pub fn new(n: usize, sample_rate: f32, cal: Calibration) -> Self {
        ...
        // Запас в 25% над физическим максимумом ITD: калибровка может быть
        // чуть шире сферической модели, а обрезать пик по краю хуже, чем
        // просмотреть несколько лишних отсчётов.
        ...
        // Кривые ILD снимаются с калибровочной сетки по азимуту, а не по
        // индексу: сетки у них разные.
    ...
    /// Сглаженный отклик по азимуту после последнего [`Localizer::analyze`].
    /// Длина — [`HIST_BINS`], шаг — [`HIST_STEP_DEG`] градусов начиная с -90.
    pub fn response(&self) -> &[f32] {
        &self.hist_smooth
    }

    /// Азимут, соответствующий узлу сетки отклика.
    pub fn response_azimuth(i: usize) -> f32 {
        -90.0 + i as f32 * HIST_STEP_DEG
    }

    /// Классический GCC-PHAT через обратное преобразование: широкополосная
    /// задержка в сэмплах (положительная — источник справа) и высота
    /// нормированного пика, 0..1.
    ///
    /// В [`Localizer::analyze`] не используется — там отклик считается сразу на
    /// сетке азимутов. Оставлен как опорная реализация и для диагностики:
    /// сырое значение задержки удобно сверять при калибровке.
    ...
    /// Полный разбор кадра: до [`MAX_PEAKS`] источников плюс отклик по всей
    /// полусфере, доступный через [`Localizer::response`].
    pub fn analyze(&mut self, lspec: &[Complex], rspec: &[Complex]) -> PeakSet {
        ...
        // В доверие к ITD идёт не вся энергия полосы, а только та, что деляция
        // сумела приписать конкретному направлению. Бины, которые не объяснил
        // ни один азимут (реверберация, шум, наложение источников), поднимать
        // доверие не должны.
        let energy_itd = self.deflate();

        // Каждая подсказка нормируется на единичную массу и лишь затем
        // взвешивается средней энергией на бин своей полосы.
        //
        // Без этого шага полоса ILD (сотни бинов) заглушала бы полосу ITD
        // (десятки) просто числом голосов, независимо от того, есть ли на
        // высоких частотах хоть какой-то сигнал. Взвешивание по средней, а не
        // суммарной энергии убирает преимущество широкой полосы: доверие к
        // подсказке становится пропорционально тому, сколько в её полосе
        // реально звучит.
    ...
    /// Отбеливает кросс-спектр в полосе ITD. Возвращает суммарную энергию
    /// полосы и число её бинов.
    fn whiten(
        &mut self,
        lspec: &[Complex],
        rspec: &[Complex],
        ...
        for (j, k) in (lo..hi.min(half)).enumerate() {
            ...
            // Фаза осмысленна, только если сигнал есть в обоих ушах.
            ...
            // Отбеливание: амплитуда отбрасывается, остаётся только фазовая
            // задержка. Это и есть PHAT.
    ...
    /// Снимает источники по одному, исключая бины, которые уже объяснены, и
    /// складывает найденные направления в [`Localizer::hist_itd`]. Возвращает
    /// суммарную приписанную энергию.
    ///
    /// Без исключения бины одного источника дают отрицательный вклад на
    /// азимуте другого и гасят его, а на полпути между источниками — наоборот,
    /// складываются в фантомный пик там, где нет никого. Это ровно та ошибка,
    /// ради устранения которой строится отклик по всей полусфере, поэтому
    /// найденное направление сразу забирает свои бины.
    fn deflate(&mut self) -> f32 {
        ...
        while found.len() < MAX_PEAKS {
            ...
            found.push(az);
            total += energy;
            // Вес голоса учитывает и энергию, и когерентность: направление,
            // объяснившее свои бины лишь наполовину, и весить должно меньше.
    ...
    /// Оценка азимута по разнице уровней, согласованная сразу по всем полосам.
    /// Возвращает энергию полосы ILD и число её бинов.
    ///
    /// Раньше каждый бин голосовал отдельно. Но полосы дают слегка разные
    /// оценки угла — и модель ILD приблизительна, и калибровка тоже, — а
    /// разброс превращался в несколько самостоятельных пиков: один источник
    /// рисовался двумя-тремя метками. Здесь считается, насколько каждый азимут
    /// объясняет измеренные ILD во всех полосах сразу, поэтому расхождение
    /// полос расширяет пик, а не плодит новые.
    fn ild_histogram(
        &mut self,
        lspec: &[Complex],
        rspec: &[Complex],
        ...
        for (bi, band) in self.cal.ild_bands.iter().enumerate() {
            ...
            for k in k_lo..k_hi {
                let (ml, mr) = (lspec[k].norm(), rspec[k].norm());
                // У жёстко панорамированного источника одно ухо почти пустое,
                // поэтому порог проверяется по громкому каналу.
            ...
            // Половина суммы, чтобы вес полосы был в той же шкале, что и
            // произведение амплитуд в полосе ITD.
            let energy = 0.5 * (el + er);
            if energy <= floor_sq {
                continue;
            }

            // Уровни усредняются по всей полосе, поэтому шум отдельных бинов
            // сюда почти не проходит.
    ...
    fn smooth(&mut self) {
        ...
        for i in 0..HIST_BINS {
            ...
            for (ki, &kv) in self.kernel.iter().enumerate() {
                ...
                // Отклик покрывает полусферу, а не окружность, поэтому
                // заворачивать края нельзя. Выходящая за край часть ядра
                // отбрасывается, а сумма нормируется на оставшуюся: если
                // вместо этого зажимать индекс, крайний узел учитывается
                // многократно и края искусственно поднимаются.
    ...
    /// Снимает пики прямо со сглаженного отклика. Используется, когда полоса
    /// ITD пуста и направление известно только по ILD.
    fn pick_peaks(&self) -> PeakSet {
        ...
        for (az, height) in candidates {
            ...
            // Доля массы отклика в пределах +-10 градусов от пика: размазанный
            // по всей полусфере отклик получает низкую уверенность даже при
            // высоком максимуме.
...
/// Уточняет положение максимума параболой по трём соседним узлам и возвращает
/// азимут в градусах.
...
/// Добавляет голос в гистограмму с линейным размазыванием между соседними
/// узлами: иначе на границе узла оценка скачет на целый шаг сетки.
...
mod tests {
    ...
    /// Один кадр анализа: окно Ханна, упаковка L+iR в одно преобразование.
    ...
    /// Источник справа: правое ухо слышит раньше, левое отстаёт на `d`.
    ...
    /// Сигнал из синусоид на заданных бинах — идеально разреженный по спектру.
    ...
    fn ild_alone_gives_direction() {
        // Сигнал выше полосы ITD: там фаза уже не помогает, работает только
        // разница уровней.
        let mut loc = Localizer::new(N, FS, Calibration::cs2());
        let bins: Vec<usize> = (100..240).collect();
        let src = tonal(999, N + 128, &bins);
        let (l0, r0) = delayed_pair(&src, 0);
        let l: Vec<f32> = l0.iter().map(|v| v * 0.5).collect(); // -6 дБ
    ...
    fn consistent_cues_yield_single_source() {
        // Так источник справа выглядит после настоящей HRTF: и задержка, и
        // разница уровней сразу.
        let mut loc = Localizer::new(N, FS, Calibration::cs2());
        let bins: Vec<usize> = (5..250).collect();
        let src = tonal(555, N + 128, &bins);
        let (l0, r0) = delayed_pair(&src, 25);
        let l: Vec<f32> = l0.iter().map(|v| v * 0.209).collect(); // -13.6 дБ
    ...
    fn separates_two_spectrally_distinct_sources() {
        // Разделение возможно, только когда источники занимают разные участки
        // спектра. При полном перекрытии фаза в каждом бине становится их
        // смесью и не принадлежит ни одному.
        let mut loc = Localizer::new(N, FS, Calibration::cs2());
